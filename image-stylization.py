...
"""
hub_model1 = hub.load('magenta_arbitrary-image-stylization-v1-256_1')
hub_model2 = hub.load('magenta_arbitrary-image-stylization-v1-256_2')
# %% 
# tensor转image图片
...
def stylization(content_path, style_path, mode = "m2"):
  ...
  if mode == "m2":
    ...
    return tensor_to_image(stylized_images[0])
# %%
def main(content_dir = "input_png", out_dir = 'out', dir_style = 'style', mode = "m2"):
  ...
  for style in styles:
    ...
    for i, n in enumerate(f_names):
      ...
      img_path = os.path.join(dir_style, style)

      ## change
      img_arr = stylization(f_path, img_path, mode)


      image_pil = PIL.Image.fromarray(img_arr)
      n_path = os.path.join(out_dir, "_"+style+"+"+mode+"-"+n)
      # print('n_path', n_path)
